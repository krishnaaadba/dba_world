...
# Desc   : This script optimizes MySQL's innodb tables online
...
cat << EOF
	...
	==============
		...
		- d : Database the table/s is/are on < Takes only one argument unlike -t > 
...
get_tables () {
	local SQL="SELECT table_name FROM information_schema.tables WHERE table_schema='$DB' AND table_type='BASE TABLE';"
